...
import (
	...
	"fyne.io/fyne/v2"
	"fyne.io/fyne/v2/cmd/janice/tooltip"
...
type toolTipContext struct {
	lock                 sync.Mutex
	toolTipHandle        *tooltip.ToolTipHandle
...
func (t *toolTipContext) setPendingToolTip(wid fyne.CanvasObject, toolTipText string) {
	...
	go func() {
		<-time.After(tooltip.NextToolTipDelayTime())
		select {
		case <-ctx.Done():
			return
		default:
			...
			t.toolTipHandle = tooltip.ShowToolTipAtMousePosition(canvas, pos, toolTipText)
		}
	}()
}
...
func (t *toolTipContext) cancelToolTip() {
	...
	if t.toolTipHandle != nil {
		tooltip.HideToolTip(t.toolTipHandle)
		t.toolTipHandle = nil
	}
}
