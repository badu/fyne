...
func TestLoad(t *testing.T) {
	...
	ui := edit.makeUI()
	w.SetContent(ui)
	r, err := storage.Reader(storage.NewFileURI("./test_data/test.txt"))
	assert.Nil(t, err)
	err = edit.load(r)
	assert.Nil(t, err)
...
func TestSave(t *testing.T) {
	...
	out, err := storage.Writer(storage.NewFileURI("./test_data/test2.txt"))
	assert.Nil(t, err)
	defer os.Remove("./test_data/test2.txt")
	...
	data, err := ioutil.ReadFile("./test_data/test2.txt")
	assert.Nil(t, err)
