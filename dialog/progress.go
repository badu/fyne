...
A progress modal can be used similar to Fyne dialogs:
	...
	m := dialog.NewProgressDialogInfinite("Loading file", "Loading file XX. Please wait.", func() error {
...
func NewProgress(title, message string, action func(progress binding.Float) error, max float64, parent fyne.Window) *ProgressModal {
	...
	m.d = NewCustomWithoutButtons(title, content, parent)
	return m
}
...
// NewProgressDialogInfinite returns a new [ProgressInfiniteModal] instance.
func NewProgressDialogInfinite(title, message string, action func() error, parent fyne.Window) *ProgressInfiniteModal {
