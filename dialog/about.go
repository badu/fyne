...
func NewAbout(content string, links []*widget.Hyperlink, a fyne.App, w fyne.Window) *CustomDialog {
	d := NewCustom("About", "OK", aboutContent(content, links, a), w)
...
func aboutFooter(links []*widget.Hyperlink) fyne.CanvasObject {
	...
	for i, a := range links {
		footer.Add(a)
		if i < len(links)-1 {
			footer.Add(widget.NewLabel(widget.LabelWithStaticText("-")))
		}
	}
