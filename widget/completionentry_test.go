...
func TestCompletionEntry_Custom(t *testing.T) {
	...
	entry.CustomCreate = func() fyne.CanvasObject {
		return NewCheck(CheckWithLabel("thing"))
	}
	entry.CustomUpdate = func(id ListItemID, o fyne.CanvasObject) {
		o.(*Check).Text = entryData[id]
		o.Refresh()
	}
	win := test.NewWindow(entry)
	win.Resize(fyne.NewSize(500, 300))
	...
	assert.Equal(t, "bar", item1.(*Check).Text) // ensure the item is a Check not Label
}
