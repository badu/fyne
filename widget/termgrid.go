...
func (t *TermGrid) CreateRenderer() fyne.WidgetRenderer {
	...
	TextGridStyleWhitespace = &CustomTextGridStyle{FGColor: theme.Color(theme.ColorNameDisabled)}

	return render
}

// NewTermGrid creates a new empty TextGrid
...
func (t *termGridRenderer) setCellRune(str rune, pos int, style TextGridStyle) {
	if str == 0 {
		str = ' '
	}
	...
	for rowIndex, row := range t.text.Rows {
		...
		if t.text.ShowLineNumbers {
			...
			for c := 0; c < len(lineStr); c++ {
				t.setCellRune(lineStr[c], x, TextGridStyleDefault) // line numbers
				i++
				x++
			}
		...
		for _, r := range row.Cells {
			...
			if t.text.ShowWhitespace && (r.Rune == ' ' || r.Rune == '\t') {
				...
				if r.Style != nil && r.Style.BackgroundColor() != nil {
					...
					t.setCellRune(sym, x, TextGridStyleWhitespace) // whitespace char
				}
			} else {
